...
class UserRegisterResource(Resource):
    def post(self):
        ...
        # 만약 비번은 4자리 이상 14자리 이하라고 한다면 이런 것을 여기서 체크한다.
        # 정상 조건 말고 비정상 조건을 적어줘서 비정상일때만 걸리게끔 한다.
...
class UserLoginResource(Resource):
    def post(self) :
        ...
        # 회원 가입을 안한 경우 리스트에 데이터가 없다.
        if len(result_list) == 0 :
            return {"error":"회원 가입을 하세요."}, 400

        # 회원이 맞으면 비밀번호가 맞는지 체크
        # 로그인한 사람이 입력한 비번 : data['password']
        # 회원 가입할때 입력했던 암호화된 비번 : DB에 있다.
        # result_list 에 들어있고 이 리스트의 첫번째 데이터에 들어있다. = result_list[0]['password']
        ...
        # 토큰 유효기간 주는 코드(config.py, JWT_ACCESS_TOKEN_EXPIRES = True)
